...
import { isTurnstileTrusted, setTurnstileTrustedCookie, TURNSTILE_TRUST_COOKIE_NAME } from "@/lib/turnstile-trust";
...
export async function GET(req: NextRequest) {
  ...
  const posts = await Post.find(baseQuery).sort(sortSpec).limit(limit + 1).lean();
  const nextCursor = posts.length > limit ? String(posts[limit - 1]?._id) : null;

  const authorUserIds = Array.from(new Set(posts.slice(0, limit).map((p) => String(p.authorUserId)).filter(Boolean)));
...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json()) as { body?: string; hp?: string; turnstileToken?: string };
  ...
  const res = NextResponse.json({ id: String(created._id) }, { status: 201 });
  if (!trusted) {
